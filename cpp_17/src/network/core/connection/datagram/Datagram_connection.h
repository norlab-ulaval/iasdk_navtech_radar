...
namespace Navtech::Networking::Datagram {
    ...
    Active::Task_state Receiver<protocol, tls_policy>::run()
    {
        ...
        try {
            ...
            stdout_log << Logging_level::debug << "Receiver - failure to read datagram: " << e.what() << endl;
            shutdown();
            return Task_state::finished;
        }
    }
    ...
    public:
        ...
        Connection(ID_Ty identifier, Socket_Ty&& sckt, Dispatcher_Ty& event_dispatcher, Direction = tx);
        ~Connection();
    ...
    Connection<protocol, tls_policy>::Connection(
        ...
        Connection<protocol, tls_policy>::Direction         direction
    ) :
        dispatcher  { associate_with(event_dispatcher) },
